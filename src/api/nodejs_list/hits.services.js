...
const deletehit = async (id) => {
  try {
    ...
    let nodeRegister_list = await nodejs_list
      .findOne({}, { created_at: 1 })
      .sort({ created_at: -1 });
    ...
    if (hitStory) {
      ...
      idRegisters.push({
        story_id: hitStory.story_id,
        objectID: hitStory.objectID,
      });
      //Delete the element
...
const serverConectHits = async (date) => {
  try {
    ...
    let lastdata = await functionService.last_data(
      nodeRegister_list,
      nodejs_log
    );
    ...
    if (empty)
      nodeRegister_list = await nodejs_list
        .findOne({}, { created_at: 1 })
        .sort({ created_at: -1 });
